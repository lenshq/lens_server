      ...
      module Sources
        class ScenariosController < Api::V1::Applications::Sources::ApplicationController
          def index
            ...
            # ...
            # ]
            rows = Event.find_by(application: application, event_source: event_source)
            scenarios = rows.group_by {|row| row['event']['scenario']}
            ...
            #   }
            # }
            quantiles = scenarios.keys.map do |scenario|
            ...
            distributions = scenarios.keys.map do |scenario|
            ...
            events = scenarios.keys.map do |scenario|
              raw_events = scenarios[scenario].map do |row|
          ...
          def show
            ...
            #   }
            # }
            raw_quantiles = Request.find_quantiles_by(application: application, scenario: scenario.events_hash).first

            raw_distributions = Request.find_distribution_by(application: application, scenario: scenario.events_hash).first
